...
void combate(int PH, int PD, int PA, int PS){ 
    ...
    while(Vida>0){ // Enquanto o jogador estiver vivo
        ...
        if(V+1<=2){
			...
			char Ename[60]="\033[1;95mCthulhu";
			PlaySound(TEXT("BossBattle.wav"), NULL, SND_FILENAME | SND_ASYNC);
		}
        ...
        while(Evida>0 && Vida>0){ // Enquanto o inimigo estiver vivo e você também
			...
			if(Vida>0){
	            switch (act){
	                case 1:
	                system("cls");
	                // comandos para chamar o codigo do ataque
	                CreateProcess(
					"AudioAttack.exe", 
					cmd, 
					NULL, 
					NULL, 
					false, 
					0, // Trocar por 0 se quiser uma só janela, porém o 1º programa não vai esperar 2º programa, quando o segundo terminar volta automatico ao primeiro.		 
					NULL, 
					NULL, 
					&sInfo, 
					&pInfo);
    ...
    if(V==16){
    	...
    	PlaySound(TEXT("BossMorte.wav"), NULL, SND_FILENAME | SND_SYNC);
	}else{
		system("cls");
...
int main()
{
	...
	PlaySound(TEXT("Zloop.wav"), NULL, SND_LOOP | SND_ASYNC);
